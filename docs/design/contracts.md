...
当发现代码与本文档冲突时:
1. **判断方向**: 是代码错 (bug) 还是文档错 (契约过时)?
2. **代码错**: 修代码, 补 property test 守卫, 在 commit message 引用契约 ID.
3. **文档错**: 修本文档, 在 commit message 注明 "docs(contracts): <ID> 因为 <原因> 调整". 若是重大调整 (语义变化), 在 [§99 变更日志](#99-变更日志) 记录.
