...
let
  ...
  providerToml =
    ...
    render (
      baseArgs
      // {
        providers = baseArgs.providers // {
          ${id} = baseArgs.providers.${id} // overrides;
        };
      }
    );
  ...
  assertions = [
    ...
    }
    {
      ...
      ok =
        domainsParsed.server.allowed_domains == [
          "a.example.com"
          "z.lan"
        ];
    }
    ...
    }
    {
      ...
      ok =
        ...
        in
        (builtins.elemAt p 1).api_key == "inline-key" && !(builtins.elemAt p 1) ? api_key_file;
    }
    {
    ...
    }
    {
      ...
      ok =
        ...
        in
        (builtins.elemAt (builtins.elemAt (builtins.fromTOML t).providers 0).routes 0).upstream_model
        == "glm-4.7";
    }
    {
      ...
      ok =
        let
          t = render (
            ...
            // {
              auth = baseArgs.auth // {
                oidc = builtins.removeAttrs baseArgs.auth.oidc [ "clientSecretFile" ];
              };
            }
          );
        in
        !(builtins.fromTOML t).auth.oidc ? client_secret_file;
    }
    {
      ...
      ok =
        let
          t = render (
            ...
            // {
              auth = baseArgs.auth // {
                oidc = builtins.removeAttrs baseArgs.auth.oidc [ "redirectUrl" ];
              };
            }
          );
        in
        !(builtins.fromTOML t).auth.oidc ? redirect_url;
    }
    {
      ...
      ok =
        ...
        in
        p.enabled == false && (builtins.elemAt p.api_keys 0).label == "opencode";
    }
    {
      ...
      ok =
        ...
        in
        (builtins.elemAt (builtins.fromTOML t).auth.api_keys 0).key == "sg_x";
    }
    {
      name = "server 段: host/port 生成, upstreamTimeouts 缺省 → 超时行不渲染 (serde default 兜底, 回归 #175 分档默认)";
      ok =
        parsed.server.port == 18787
        && parsed.server.host == "127.0.0.1"
        && !(parsed.server ? upstream_response_header_timeout_secs);
    }
    {
    ...
    }
    {
      ...
      ok =
        let
          t = render (baseArgs // { secretsEntries = [ ]; });
        in
        !lib.hasInfix "secrets.entries" t;
    }
    {
      name = "auth=null → 不渲染 [auth] 段";
      ok =
        let
          t = render (baseArgs // { auth = null; });
        in
        !lib.hasInfix "[auth]" t;
    }
    ...
    }
    {
      ...
      ok =
        ...
        in
        pos ''[usage.pricing_override."glm-5.3"]'' < pos ''[usage.pricing_override."zhipuai/glm-5.3"]'';
    }
    {
      name = "usage: 含 '/' 键 quoted-key round-trip 不损";
      ok =
        let
          o = parsed.usage.pricing_override."zhipuai/glm-5.3";
        in
        o.input == 1.4 && o.output == 4.4 && o.cache_read == 0.26 && o.cache_write == 0.0;
    }
    {
    ...
    }
    {
      ...
      ok =
        ...
        in
        p.enabled == false && !(p ? pricing_override);
    }
    {
    ...
    }
    {
      ...
      ok = fails (
        render (
          ...
          // {
            providers = baseArgs.providers // {
              "bad id" = baseArgs.providers.b-upstream;
            };
          }
        )
      );
    ...
    }
    {
      ...
      ok = renderFails {
        auth = baseArgs.auth // {
          oidc = baseArgs.auth.oidc // {
            redirectUrl = "https://sg.example.com/wrong/path";
          };
        };
      };
    }
