...
#
# 输入为结构化 attrset (nix/module.nix 的 providers / secrets.entries / auth 选项
# 形态), 输出为 secret-guard.toml 字符串. 字段序/注释/排序规则只在此处声明一次,
# 消费侧 (module.nix 自动 render + 本文件契约测试) 只传结构化数据 — 新增 provider
# 只需设选项, 消灭"手抄 toml 模板 + mkForce 覆盖"模式.
#
