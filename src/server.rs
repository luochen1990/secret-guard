...
///
/// TCP keepalive: 上游连接的**活性判官** (2026-09-23 裁决, 与应用层超时的职责分工).
/// prefill / 思考期连接完全空闲, 恰是探测的理想战场 — 网络分区 / NAT 黑洞下
/// 探测包无响应, 内核在亚分钟级 (~45s: idle 15s + Linux TCP_USER_TIMEOUT 30s)
/// 判死连接 → `send()`/读流返回网络错误 → 502; 活着的慢连接永不误伤. 顺带
