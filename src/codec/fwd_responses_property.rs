...
//! - 流式翻译的 property (STR-*/FWD-1 流式半段): 已由 fwd_streaming_property.rs
//!   补齐 (T5) — `arb_responses_sse_stream_with_mock` 生成器 + STR-1/STR-2/
//!   FWD-1/RED-7 断言 (r→r restore 切分等价 + scan 累积 + 跨协议四组合 +
//!   restore 无泄漏); 端到端行为另由集成测试锁定
//!   (同协议 restore / r⇄o / a→r, `tests/integration.rs`).
