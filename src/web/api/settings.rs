...
//! # 字段语义
//!
//! - `audit_capture` (详细日志): 开启时新请求记录完整 req_body_raw /
//!   raw_resp_body (排障用, 内存开销大); 关闭时极致省内存 (timeline 不受影响,
//!   B1 起从 BlockPool 派生). per-request 原子语义 + 持久化契约见
//!   `crate::state::AuditCapture` 文档 (SSOT).
//!
//! # 安全
//!
...
pub async fn get_settings(State(state): State<AppState>) -> impl IntoResponse {
    (
        ...
        JsonResponse(SettingsResponse {
            audit_capture: state.audit_capture.enabled(),
        }),
    )
}

/// 切换设置: 持久化 state.toml + 更新内存 (原子, 见 [`AuditCapture::set_enabled`]).
///
...
pub async fn update_settings(
    ...
    state
        .audit_capture
        .set_enabled(payload.audit_capture)
