...
async fn fanout_stream_task(
    ...
    // raw_resp_body 照填 (finalize_body 的 move 产出零拷贝); audit_capture 三态
    // 的去留决策统一收口在 `dag::attach_response` (errors 档成功回收 / off 丢弃).
        ...
        move |parsed_sync, recorder| {
            ...
            if streamed {
                match (parsed_sync, codec_proto) {
                    (Some(ps), Some(cp)) => {
                        super::recorder::RespFinality::from_scan(cp, ps.finalize())
                    }
                ...
                match parsed_ir {
                    Some(ir) => {
                        ...
                        let finality = super::recorder::RespFinality::from_ir(cp, &ir);
...
pub(crate) async fn fan_out_buffered_ir(
    ...
    // B1: finalize 统一产出 (message + 元字段 + 回显摘要; parsed 渲染派生, 不再长期
    // 存 Value). echo 在 restore 前提取 (restore 只改字符串叶子, 不触碰 usage 数字,
    // 保持 "LLM 原始回显" 语义).
    ...
    let client_bytes: Vec<u8> = if recorder.error_kind.is_some() {
        ...
        match serde_json::from_slice::<serde_json::Value>(&recorder.acc) {
            ...
            Err(e) => {
                // 非 JSON body (eg SSE-shaped 多帧): 叶子级兜底以单 JSON Value 为
                // 前提, 对同一字节再 parse 必然同样失败 — 不再尝试, 直接透传
                // (已知限制, 见根 AGENTS.md).
    ...
    // acc_text 照填 (move 零拷贝); audit_capture 三态的去留决策统一收口在
    // `dag::attach_response` — acc 的瞬时累积本就不受影响 (pool 检测 /
    // client_bytes 派生照常).
...
fn spawn_restore_fanout(
    ...
    tokio::spawn(fanout_stream_task(
        ...
        move |parsed_sync, _recorder| {
            ...
            super::recorder::RespFinality::from_scan(writer_proto, ps.finalize())
        },
        |recorder| {
            if recorder.overflow {
                super::TRUNCATED_BANNER.to_string()
