...
/// M3 集成测试 spawn helper: `[redact] inject_cache_control = true` 的 AppState
/// (struct-update 覆盖 base 默认 false, 与 spawn_proxy_with_redact_gates 同型).
async fn spawn_proxy_with_inject_cache_control(
    providers: Vec<Provider>,
    secrets: SecretTable,
    upstream: reqwest::Client,
    records: ConversationDag,
    ...
    let provider_table = ProviderTable::with_persist_lock(
        ...
        vec![],
        decisions.clone(),
        tmp_state_path("sg-providers-inject-cc"),
        persist_lock.clone(),
    );
    let _ = (decisions, persist_lock);
    let proxy = AppState {
...
/// M3 (#269): Anthropic egress + opt-in 时, IR 路径对无缓存标记的请求注入顶层
/// automatic caching 标记; 客户端已带任何 cache_control 时不注入 (守卫), 且
/// block 级标记由 M1 保真.
#[tokio::test]
async fn anthropic_ir_path_injects_top_level_cache_control_when_enabled() {
    let mut upstream = spawn_mock_upstream().await;
    let _m = upstream
        .mock("POST", "/v1/messages")
        .with_status(200)
    ...
    // secret 命中 → 强制 same-proto IR 路径 (M3 的作用域).
    let real_secret = "sk-test-123";
    let secrets = test_secret_table_with(vec![secret("s1", real_secret)]);
    let dag = ConversationDag::new(64, 500, 1);
    let dag_probe = dag.clone();
    let proxy_url = spawn_proxy_with_inject_cache_control(
        vec![provider_with(
            "an-main",
            Protocol::Anthropic,
            &upstream.url(),
    ...
    // case 1: 客户端无任何缓存标记 → egress 注入顶层 automatic 标记.
    let (status, _, _) = proxy_request(
        &proxy_url,
        "POST",
        "/a/an-main/v1/messages",
        r#"{"model":"ultra","max_tokens":16,"messages":[{"role":"user","content":"hello sk-test-123"}]}"#,
        &[],
    )
    .await;
    assert_eq!(status, reqwest::StatusCode::OK);
    ...
    let sent: serde_json::Value =
        serde_json::from_str(&dag_probe.get_node_detail(id).unwrap().req_body_raw).unwrap();
    assert_eq!(
        sent.get("cache_control"),
        Some(&serde_json::json!({"type": "ephemeral"})),
        "无标记请求应注入顶层 automatic cache_control"
    );

    // case 2: 客户端自带 block 级标记 → 不注入 (守卫), block 标记由 M1 保真.
    let (status, _, _) = proxy_request(
        &proxy_url,
        "POST",
        "/a/an-main/v1/messages",
        r#"{"model":"ultra","max_tokens":16,"system":[{"type":"text","text":"sys","cache_control":{"type":"ephemeral"}}],"messages":[{"role":"user","content":"hello"}]}"#,
        &[],
    )
    .await;
    assert_eq!(status, reqwest::StatusCode::OK);
    ...
    let sent: serde_json::Value =
        serde_json::from_str(&dag_probe.get_node_detail(id).unwrap().req_body_raw).unwrap();
    assert!(
        sent.get("cache_control").is_none(),
        "客户端已有缓存标记 (4 断点槽守卫), 不得再注入顶层标记"
    );
    assert_eq!(
        sent["system"][0]["cache_control"],
        serde_json::json!({"type": "ephemeral"}),
        "block 级标记由 wire fidelity 保真"
    );
}

/// M3 (#269): inject_cache_control 默认关闭 — IR 路径不注入 (显式 opt-in 语义).
#[tokio::test]
async fn anthropic_ir_path_no_cache_injection_by_default() {
    let mut upstream = spawn_mock_upstream().await;
    let _m = upstream
        .mock("POST", "/v1/messages")
        .with_status(200)
    ...
    let dag = ConversationDag::new(64, 500, 1);
    let dag_probe = dag.clone();
    let (proxy_url, _) = spawn_proxy_static_dynamic(
        vec![provider_with(
            "an-main",
            Protocol::Anthropic,
            &upstream.url(),
        ...
        reqwest::Client::new(),
        dag,
        test_secret_table_with(vec![secret("s1", "sk-test-123")]),
    )
    .await;
    ...
    let (status, _, _) = proxy_request(
        ...
        "POST",
        "/a/an-main/v1/messages",
        r#"{"model":"ultra","max_tokens":16,"messages":[{"role":"user","content":"hello"}]}"#,
        &[],
    )
    .await;
    assert_eq!(status, reqwest::StatusCode::OK);
    ...
    let sent: serde_json::Value =
        serde_json::from_str(&dag_probe.get_node_detail(id).unwrap().req_body_raw).unwrap();
    assert!(sent.get("cache_control").is_none(), "默认不注入");
}
